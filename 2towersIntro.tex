...
The observation of the exact nonperturbative coincidence between the spectrum
of the kinks in the two-dimensional CP$(N-1)$ models with twisted masses and the spectrum of the monopole-dyons
in the four-dimensional Seiberg--Witten solution \cite{SeibergWitten}
dates back to 1998 \cite{Dorey:1998yh}. 
This observation was understood and interpreted in clear-cut physical terms after the discovery of non-Abelian strings 
\cite{1,2} on the Higgs branch of ${\mathcal N}=2$ Yang--Mills and the discovery of confined monopoles attached to them \cite{3,4} (represented by kinks in the effective theory on the string world sheet). It is crucial that
the monopole-dyon mass  on the Coulomb branch of super-Yang--Mills is given by the same formula  as the mass on the Higgs branch.
The underlying reason was explained in \cite{3}. 
The above results  set the stage for the development of the 2D--4D correspondence in this particular problem.

An immediate consequence of the spectral coincidence in two and four dimensions (in the Bogomol'nyi--Prasad--Sommerfield \cite{BPS}, BPS for short, sectors) is the coincidence of the curves of marginal stability (CMS) or the domain wall crossings.
These curves, being established, in two dimensions, for kinks, can be immediately elevated to four dimensions, for monopole-dyons. Generally speaking, for non-Abelian strings in SU$(N)$ gauge theories there are $N$ appropriate mass parameters.
Correspondingly, there are $N$ twisted mass parameters in the two-dimensional CP$(N-1)$ models (considered in the gauge formulation). Each mass parameter is a complex number. Therefore, in the general case one has to deal with highly multidimensional 
domain wall crossings.

In \cite{Gorsky} it was pointed out that extremely beneficial for various physical application was a special choice 
\beq
m_k = m_0\exp\left(\frac{2\pi\,i\, k}{N}\right)\,, \qquad k=0,1, ..., N-1
\label{znmasses}
\eeq
preserving a discrete $Z_N$ symmetry of the model. Then the only adjustable mass parameter $m_0$ presents a single complex number, and the domain wall crossings reduce to a number of CMS on the complex plane of $m_0$.
We will refer to (\ref{znmasses}) as the $Z_N$ symmetric masses.
...
CP$(N-1)$ (with $N\geq 3$) was addressed in \cite{5,6}. A detailed study carried out in \cite{6} revealed
the existence of a richer kink spectrum than was originally anticipated \cite{Dorey:1998yh}. In particular, the existence of $N$ towers was argued replacing  a single tower discussed in \cite{Dorey:1998yh}. However, shortly after, we realized
that we had analyzed only necessary conditions for these towers to exist, leaving the sufficient conditions aside.
In this paper we close this gap. We find that, although (based on the analyses of the central charges) $N$ towers could exist, 
...
The second tower for odd $N$ appears as the bound states of the appropriate kink with the quantum of
elementary excitation carrying no topological number.  In four-dimensional language this is a bound (s)quark.

On the four-dimensional side the general theory of the wall crossing was worked out in \cite{koso}. Its consequences
for the monopole-dyons in the case of the $Z_N$ symmetric masses (\ref{znmasses}) are studied\,\footnote{We are grateful to the authors for providing us with a draft of their paper prior to its publication.}
in \cite{ndkp}. Our task is to analyze the kink spectrum and the corresponding CMS in two dimensions,
taking account of the sufficient conditions mentioned above, and compare the result with that in four dimensions, thus  demonstrating the power of 2D--4D correspondence.
...
{\em BPS Spectrum of Supersymmetric CP$(N-1)$ Theory with $Z_N$ Twisted Masses,}
  Phys.\ Rev.\ D {\bf 84}, 085004 (2011)
  [arXiv:1104.5241 [hep-th]].
  %%CITATION = ARXIV:1104.5241;%%
  
\bibitem{Dorey:1999zk}
  N.~Dorey, T.~J.~Hollowood, D.~Tong,
{\em The BPS spectra of gauge theories in two-dimensions and four-dimensions,}
...
{\em Supersymmetric Solitons and How They Help Us Understand Non-Abelian Gauge Theories,}
  ...
  [hep-th/0703267]; fo r an extended version see   M.~Shifman, A.~Yung, {\em Supersymmetric Solitons },
