...
\end{center}
	...
	We perform an independent analysis of the ``extra'' states found previously in \cite{Bolokhov:2011mp}
	and compare them to the existing bound states.
	Exact analysis of the strong-coupling states shows that they do not survive when passing into the
	weak-coupling regime and thus do not match any of the observable bound states.

	In the weak-coupling sector, we re-confirm the existence of bound states 
	...
	only two actually survive in the spectrum for odd $ N $, and a single tower for even $ N $.
...
%			O L D E R  V E R S I O N
%
%	We continue to study the  BPS spectrum of the ${\mathcal N}=(2,2)$ CP$^{N-1}$ model  with 
%	the $ \mc{Z}_N $-symmetric twisted mass terms,
%	with emphasis  on  semiclassical analysis at large masses.
%	We re-confirm the existence of bound states of topologically-charged kinks and elementary quanta.
%	Quantization of the U(1) kink modulus leads to formation of towers of such states. 
%	Dynamical investigation in the quasiclassical limit shows that out of $N$ possible towers  
%	only two actually survive in the spectrum for odd $ N $, and a single tower for even $ N $.
%
...
\label{sstates}
	\W_1 ~~-~~ \W_0   ~~+~~ i\, m_k\,,   \qquad\qquad k ~=~ 0\,,~1\,,~2\,.
\eeq
	If one naively calculates their masses at large $ m_0 $, {\it e.g.} using Eq.~\eqref{weak},
	one would find that the lightest state is $ \W_1 ~-~ \W_0 ~+~ i\, m_2 $.
	However, as we will show, this state decays at strong coupling, and therefore cannot exist at large masses.
	Among the other two states, the lightest is
\beq
\label{monopole}
	\M ~~=~~ \W_1 ~~-~~ \W_0   ~~+~~ i\, m_0\,,
\eeq
	...
	The other third state will be a dyonic state, and thus, heavier at the weak coupling.
...
\eeq
	...
	the other two states of the strong coupling now have the quantum numbers 
\beq
\label{twostates}
	\D  ~~=~~  \M ~~+~~ \Q_{10}  \qquad\qquad  \text{and}  \qquad\qquad   
	\V^2  ~~=~~  \M ~~+~~ \Q_{20}\,.
\eeq
	The first state will be identified as the dyon in four-dimensions.
	For convenience, we will loosely call the state $ \M $ a ``monopole'', 
	the state $ \Q_{ik} $ a quark and the state $ \D $ a dyon in two dimensions too.
	The other state in Eq.~\eqref{twostates} does not have a quasiclassical interpretation.
...
\eeq
	...
	Let us see this.
	For $ k \,=\, 0 $ and $ n \,=\, 0 $, Eq.~\eqref{weak} provides the quasi-classical approximation for the monopole mass,
\beq
...
\eeq
	The extra 1/2 is not captured at the quasiclassical level of precision, as its correction to the mass
	...
	In our case of CP$^2$, there is only one appropriate fermion --- $ \psi^2 $. 
	...
	Even if it did, its mass would be \emph{lighter} than that of the monopole as is easy to find.
	Indeed, the mass of such a state in quasiclassics would be described by a limit of Eq.~\eqref{weak}
...
\eeq
	...
	Therefore, $ \V_{10}^2 $ could not possibly qualify for a bound state of the monopole and a quark.

	The semiclassical expression for the mass of the bound state is found using the eigenvalue from Eq.~\eqref{E},
\beq
\label{kinkpsi}
...
\eeq
	In fact, this is the mass of a bound state of the monopole and an anti-state
$ \Q_{02} ~=~  -\, \Q_{20} $,
...
\eeq
	as well as their dyonic excitations $ \D_{10}^{(n)}\,\Q_{02} $.
...
\eeq
	...
	Spectrum \eqref{wspectrum} must change to \eqref{sspectrum} on the way from the weak coupling to the strong coupling.
...
\eeq
	...
	Dyons with $ n ~=~ 0 $ and $ n ~=~ 1 $ are the lightest, they do not decay and become the strong coupling
...
\setcounter{equation}{0}
	...
	cannot be described semiclassically.
