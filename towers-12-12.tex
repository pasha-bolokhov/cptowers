...
{\large\bf Abstract}
\end{center}
	We continue the analysis of BPS spectrum of CP$^{N-1}$ theory with $ \mc{Z}_N $ twisted mass terms,
	with an accent on semiclassical analysis.
	The extra states found previously are shown to be bound states of topological states and elementary quanta.
	Quantization of the U(1) coordinate of the kinks leads to formation of towers of such states.
	Unlike previously predicted, only one extra tower of such states exists for odd $ N $, 
	and none for even $ N $.
...
\cite{Dorey:1998yh}. 
	...
	\cite{1,2} on the Higgs branch of ${\mathcal N}=2$ Yang--Mills and the discovery of confined monopoles attached to them 
\cite{Shifman:2004dr,4} 
	(represented by kinks in the effective theory on the string world sheet). 
	It is crucial that the monopole-dyon mass  on the Coulomb branch of super-Yang--Mills is given by the same formula  
	as the mass on the Higgs branch.
...
\eeq
	...
	and the domain wall crossings reduce to a number of CMS on the complex plane of $m_0$.
	...
	In particular, the existence of $N$ towers was argued replacing  a single tower discussed in \cite{Dorey:1998yh}. 
	...
	an elementary excitation carrying no topological number.  In four-dimensional language this is a bound (s)quark.

	On the four-dimensional side the general theory of the wall crossing was worked out in \cite{koso}. 
	Its consequences for the monopole-dyons in the case of the $Z_N$ symmetric masses (\ref{znmasses}) 
...
\eeq
	Here $ s $ is chosen once and globally.
...
\eeq
	...
	As indicated above, in four dimensions this would correspond to the bound states of a monopole and quarks.
...
\eeq
	...
	We re-instate that the physical values of the central charge 
\beq
\label{ccharge}
...
\cite{Dorey:1998yh}.
	...
	Importantly, only ``strong-coupling bound state'' is seen in quasiclassics. 
...
%%TEMPCOMM%%
%%TEMPCOMM%%
%%TEMPCOMM%%       We will be looking for the semi-classical expression for the central charge in the presence
 ...
 \nonumber\\[3mm]
  ...
  \nonumber\\[3mm]
	&=&
	  ...
	  \right).
\end{eqnarray}
%%TEMPCOMM%%\marginpar{\tiny The order of the limits is reversed in the formula above}
...
\eeq
	...
	``non-perturbative'' quantity, as being part of the exact superpotential.
...
\eeq
	...
	between the vacua (0) and (1), with the rest of the spectrum recoverable by action of $ \mc{Z}_3 $ symmetry.
...
\beq
	\mc{D}^{(n)}  ~~\to~~  \mc{D}^{(n)} ~~+~~ \mc{Q}_2\,,
\eeq
	and
\beq
	\mc{D}^{(n)}  ~~\to~~  \mc{D}^{(n+1)} ~~+~~ \mc{Q}_{21}\,,
\eeq
...
\setcounter{equation}{0}
	...
	The elementary states, which correspond to quarks in four dimensions, decay on the primary CMS.
	...
	The correspondence between the spectra of the two-dimensional sigma model and four-dimensional gauge theory
	at the root of the Higgs branch elevates this picture into four dimensions.
...
{\em Stability structures, motivic DonaldsonÐThomas invariants and cluster transformations},
